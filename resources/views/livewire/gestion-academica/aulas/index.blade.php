<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-semibold text-gray-900 dark:text-white">Gestión de Aulas</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Administra las aulas de la facultad</p>
        </div>
            ...
            <table class="w-full bg-white dark:bg-gray-900">
                <thead class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-200">Capacidad</th>
                        <th class="px-6 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-gray-700 dark:text-gray-200">Ubicación</th>
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                    @forelse ($aulas as $aula)
                        <tr class="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                            <td class="whitespace-nowrap px-6 py-4">
                                <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $aula->codigo }}</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-900 dark:text-gray-100">{{ $aula->nombre }}</span>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <span class="text-sm text-gray-700 dark:text-gray-300">{{ $aula->capacidad }} personas</span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="text-sm text-gray-700 dark:text-gray-300">{{ $aula->ubicacion ?? 'N/A' }}</span>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4">
                                <button wire:click="toggleActivo({{ $aula->id_aula }})"
                                    class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold transition
                                        {{ $aula->activo ? 'bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300' }}">
                            ...
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    ...
                                    <button wire:click="confirmDelete({{ $aula->id_aula }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">No se encontraron aulas</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-2xl max-w-md w-full border border-gray-200 dark:border-gray-700">
                <div class="p-6">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            <h3 class="text-lg font-semibold text-gray-900 dark:text-white">¿Eliminar Aula?</h3>
