<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Gestiona las asignaciones de docentes, grupos y horarios</p>
        </div>
    ...
    <div class="max-w-md">
        <input 
            ...
            placeholder="Buscar por materia, grupo o docente..."
        ...
        <div class="overflow-x-auto">
            <table class="w-full bg-white dark:bg-gray-900">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr class="bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 transition">
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $asignacion->materia->codigo }}</span>
                                    <span class="text-xs text-gray-600 dark:text-gray-400">{{ $asignacion->materia->nombre }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex flex-col">
                                    <span class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ $asignacion->grupo->codigo }}</span>
                                    <span class="text-xs text-gray-600 dark:text-gray-400">{{ $asignacion->grupo->nombre }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="text-sm text-gray-900 dark:text-gray-100">{{ $asignacion->docente->nombre }}</span>
                                </div>
                            </td>
                            ...
                            <td class="whitespace-nowrap px-6 py-4">
                                <button wire:click="toggleActivo({{ $asignacion->id_mg }})"
                                    class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold transition
                                        {{ $asignacion->activo ? 'bg-green-100 dark:bg-green-900/40 text-green-800 dark:text-green-300' : 'bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300' }}">
                                    {{ $asignacion->activo ? 'Activo' : 'Inactivo' }}
                                </button>
                            </td>
                            <td class="whitespace-nowrap px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-3">
                                    <a href="{{ route('asignar-horarios.edit', $asignacion->id_mg) }}" 
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">No se encontraron asignaciones</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
                ...
                <div class="p-6">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Esta acción no se puede deshacer y puede afectar los horarios asociados.</p>
                        </div>
                    </div>
                </div>
